...
- Fixed issues where ChikLisp was compiled during import requiring write access to the directory (#11257) (thanks @lourkeur). To force compilation, developers can set environment variable `CHIK_DEV_COMPILE_CLVM_ON_IMPORT`
...
- Added `chik wallet nft` command (see <https://docs.chik-network.net/docs/13cli/did_cli>)
- Added `chik wallet did` command (see <https://docs.chik-network.net/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.chik-network.net/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.chik-network.net/docs/12rpcs/nft_rpcs>)
...
- Beta builds are built on every merge to main, and are now available from <https://chik-network.net/download/>.
...
- Switched IP address lookup to first use Chik's service ip.chik-network.net.
...
- This is a hard fork/breaking change from RC6/7. TXCK Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chik-network.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We have moved to a new and much higher scalability download.chik-network.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.chik-network.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.chik-network.net/latest/Setup-MacOS.dmg).
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.chik-network.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.chik-network.net via https, for example.
...
- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 TXCK as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/Chik-Network/clvm_rs) is complete.
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and ChikLisp code matches the CI compiled code.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [chik-network.net](https://chik-network.net/). Among the improvements this gives the Chik blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- Partial support for Python 3.9. That includes new versions of Chik dependencies like chiabip158.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Chik Network at backup.chik-network.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Chik backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chik-network.net, your own installation, or a third party's version of it.
...
- There was a regression in Chik Proof of Space ([chiapos](https://github.com/Chik-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- More details on the release at [https://www.chik-network.net/developer/](https://www.chik-network.net/developer/)
