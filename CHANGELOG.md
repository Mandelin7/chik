...
- Added `chik wallet nft` command (see <https://docs.chiknetwork.org/docs/13cli/did_cli>)
- Added `chik wallet did` command (see <https://docs.chiknetwork.org/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.chiknetwork.org/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.chiknetwork.org/docs/12rpcs/nft_rpcs>)
...
- Beta builds are built on every merge to main, and are now available from <https://chiknetwork.org/download/>.
...
- Switched IP address lookup to first use Chik's service ip.chiknetwork.org.
...
- This is a hard fork/breaking change from RC6/7. TXCK Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chiknetwork.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We have moved to a new and much higher scalability download.chiknetwork.org to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.chiknetwork.org/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.chiknetwork.org/latest/Setup-MacOS.dmg).
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.chiknetwork.org/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.chiknetwork.org via https, for example.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Chik Network at backup.chiknetwork.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Chik backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chiknetwork.org, your own installation, or a third party's version of it.
...
- More details on the release at [https://www.chiknetwork.org/developer/](https://www.chiknetwork.org/developer/)
