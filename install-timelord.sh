...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("chikvdf").location)' 2>/dev/null)/vdf_client
CHIKVDF_VERSION=$(python -c 'import os; os.environ["CHIK_SKIP_SETUP"] = "1"; from setup import dependencies; t = [_ for _ in dependencies if _.startswith("chikvdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test "$UBUNTU_DEBIAN"; then
		...
		echo "Installing chikvdf from source on Ubuntu/Debian"
		echo venv/bin/python -m pip install --force --no-binary chikvdf "$CHIKVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary chikvdf "$CHIKVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test "$RHEL_BASED"; then
		echo "Installing chikvdf dependencies on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing chikvdf from source on RedHat/CentOS/Fedora"
		echo venv/bin/python -m pip install --force --no-binary chikvdf "$CHIKVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary chikvdf "$CHIKVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test "$MACOS"; then
		echo "Installing chikvdf dependencies for MacOS."
