...
root = here.parent
cache_path = root.joinpath(".chik_cache", "manage_klvm.json")
...
clsp_suffix = ".clsp"
hex_suffix = ".clsp.hex"
all_suffixes = {"clsp": clsp_suffix, "hex": hex_suffix, "klvm": klvm_suffix}
...
class ManageKlvmError(Exception):
    pass
...
class CacheVersionError(ManageKlvmError):
    pass
...
class KlvmPaths:
    ...
    @classmethod
    def from_klvm(cls, klvm: pathlib.Path, hash_dict: typing.Dict[str, str] = {}) -> KlvmPaths:
        stem_filename = klvm.name[: -len(clsp_suffix)]
        hex_path = klvm.with_name(stem_filename + hex_suffix)
        missing_files = []
...
class KlvmBytes:
    ...
    @classmethod
    def from_klvm_paths(cls, paths: KlvmPaths, hash_dict: typing.Dict[str, str] = {}) -> KlvmBytes:
    ...
    @classmethod
    def from_hex_bytes(cls, hex_bytes: bytes) -> KlvmBytes:
        return cls(
            hex=hex_bytes,
...
def create_cache_entry(reference_paths: KlvmPaths, reference_bytes: KlvmBytes) -> CacheEntry:
    source_bytes = reference_paths.klvm.read_bytes()

    klvm_hasher = hashlib.sha256()
    klvm_hasher.update(source_bytes)
...
def check(use_cache: bool) -> int:
    ...
    print()
    print("Checking that no .klvm files begin with `(mod`")
    for stem_path in sorted(found_stems["klvm"]):
        with open(stem_path.with_name(stem_path.name + klvm_suffix)) as file:
            ...
            for line in file_lines:
                ...
                if "(" in non_comment:
                    ...
                    if len(non_comment) >= paren_index + 4 and non_comment[paren_index : paren_index + 4] == "(mod":
                        overall_fail = True
                        print(f"FAIL    : {stem_path.name + klvm_suffix} contains `(mod`")
                    break
    ...
    for stem_path in sorted(found_stems["clsp"]):
        ...
        try:
            reference_paths = KlvmPaths.from_klvm(klvm=clsp_path, hash_dict=HASHES)
            ...
            reference_bytes = KlvmBytes.from_klvm_paths(paths=reference_paths, hash_dict=HASHES)
            ...
            if not cache_hit:
                with tempfile.TemporaryDirectory() as temporary_directory:
                    generated_paths = KlvmPaths.from_klvm(
                        klvm=pathlib.Path(temporary_directory).joinpath(reference_paths.klvm.name),
                        hash_dict=HASHES,
                    )

                    compile_klvm(
                        input_path=os.fspath(reference_paths.klvm),
                        output_path=os.fspath(generated_paths.hex),
                        search_paths=[os.fspath(reference_paths.klvm.parent), str(std_libraries)],
                    )

                    generated_bytes = KlvmBytes.from_hex_bytes(hex_bytes=generated_paths.hex.read_bytes())
    ...
    if missing_files != []:
        overall_fail = True
        print()
        print("Missing files (run tools/manage_klvm.py build to build them):")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temporary_directory:
                ...
                if reference_paths.hash not in new_hashes:
                    new_hashes[reference_paths.hash] = KlvmBytes.from_klvm_paths(
...
main(auto_envvar_prefix="CHIK_MANAGE_KLVM")
