...
version_data = copy_metadata(get_distribution("chik-blockchain"))[0]
...
# TODO: collapse all these entry points into one `chik_exec` entrypoint that accepts the server as a parameter

entry_points = ["chik.cmds.chik"] + [f"chik.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "chik.util.bip39"])
...
datas.append((f"{ROOT}/chik/wallet/puzzles/*.hex", "chik/wallet/puzzles"))
