...
from chik.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path):
    from chik.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'chik plots add -d [dir]' and remove with"
        + " 'chik plots remove -d [dir]'"
        + " Scan and check plots with 'chik plots check'"
    )
    print()
...
def create_cmd(
    ...
    from chik.plotting.util import Params
...
def check_cmd(
    ...
    from chik.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from chik.plotting.util import remove_plot_directory
