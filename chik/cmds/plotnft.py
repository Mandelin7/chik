...
def validate_fee(ctx: click.Context, param: click.Parameter, value: str) -> str:
    try:
        ...
        raise click.BadParameter("Fee must be decimal dotted value in XCK (e.g. 0.00005)")
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XCK. Fee is used TWICE: once to leave pool, once to join.",
    type=str,
    default="0",
    show_default=True,
    required=True,
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XCK. Fee is charged TWICE.",
    type=str,
    default="0",
    show_default=True,
    required=True,
