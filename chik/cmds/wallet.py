...
from chik.cmds.cmds_util import execute_with_wallet
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in XCK", type=str, required=False)
def _mint_vc(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option("-m", "--fee", help="Blockchain fee for update transaction, in XCK", type=str, required=False)
@click.option(
...
@click.option("-m", "--fee", help="Blockchain fee for revocation transaction, in XCK", type=str, required=False)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
    default=False,
