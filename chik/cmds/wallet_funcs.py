...
from chik.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
...
from chik.server.start_wallet import SERVICE_NAME
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["chik"])
            ...
            if confirmation not in ["y", "yes"]:
                ...
                with open(pathlib.Path(filepath), "w") as file:
                    ...
                    if offer is not None:
                        ...
                        print(
                            f"Use chik wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status"
                        )
                    else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_xck="XCK"
):
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    from chik.wallet.util.puzzle_compression import decompress_object_with_puzzles
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.XCK}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.XCK}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.XCK}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["chik"])
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["chik"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["chik"])
...
async def spend_clawback(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    if fee < 0:
        ...
        return
    response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["chik"]))
    print(str(response))
...
async def mint_vc(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    vc_record, txs = await wallet_client.vc_mint(
        ...
        if args["target_address"] is None
            ...
            ensure_valid_address(args["target_address"], allowed_types={AddressType.XCK}, config=config)
        ),
        uint64(0) if args["fee"] is None else uint64(int(Decimal(args["fee"]) * units["chik"])),
    )
...
async def spend_vc(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    txs = await wallet_client.vc_spend(
        ...
        fee=uint64(0) if args["fee"] is None else uint64(int(Decimal(args["fee"]) * units["chik"])),
        reuse_puzhash=args["reuse_puzhash"],
    )
