...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        1, uint64(1), encode_puzzle_hash(await wallet.get_new_puzzlehash(), "txck")
    )  # Create a pending tx
    ...
    await client.send_transaction(1, uint64(1), encode_puzzle_hash(ph_by_addr, "txck"))
    await client.farm_block(encode_puzzle_hash(ph_by_addr, "txck"))
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
...
async def test_offer_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    assert id == offer.name()
    assert summary == {"offered": {"xck": 5}, "requested": {cat_asset_id.hex(): 1}, "infos": driver_dict, "fees": 1}
...
async def test_nft_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    res = await wallet_1_rpc.mint_nft(
        ...
        ["https://www.chik-network.net/img/branding/chik-logo.svg"],
    )
    assert res["success"]
...
# xck1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
# $ chik wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a xck1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
#
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            # -a xck1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
        ...
        (
            ...
            # -a xck1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "xck1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh",
            },
            {"isValid": True},
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "xck1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gdsjen7t0",
            },
