...
async def test_nft_mint_from_did(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata_list = [
        {
            "program": Program.to(
                [("u", ["https://www.chiknetwork.com/img/branding/chik-logo.svg"]), ("h", token_bytes(32).hex())]
            ),
...
async def test_nft_mint_from_did_rpc(
    ...
    try:
        ...
        royalty_address = encode_puzzle_hash(bytes32(token_bytes(32)), "xck")
        royalty_percentage = 300
        fee = 100
        required_amount = n + (fee * n)
        xck_coins = await client.select_coins(amount=required_amount, wallet_id=wallet_maker.id())
...
@pytest.mark.asyncio
async def test_nft_mint_from_did_multiple_xck(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    xck_coins_1 = await wallet_maker.select_coins(amount=10000)
    xck_coins_2 = await wallet_maker.select_coins(amount=10000, exclude=xck_coins_1)
...
@pytest.mark.asyncio
async def test_nft_mint_from_xck(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    sb = await nft_wallet_maker.mint_from_xck(
        metadata_list, target_list=target_list, mint_number_start=1, mint_total=mint_total, fee=fee
    )
...
@pytest.mark.asyncio
async def test_nft_mint_from_xck_multiple_xck(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
