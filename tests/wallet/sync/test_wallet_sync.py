...
class TestWalletSync:
    ...
    async def test_dusted_wallet(
        ...
        for i in range(large_coins):
            payee_ph = await dust_wallet.get_new_puzzlehash()
            payees.append({"amount": uint64(xck_spam_amount), "puzzlehash": payee_ph, "memos": []})
        ...
        for i in range(5):
            ...
            payees.append({"amount": uint64(xck_spam_amount + i), "puzzlehash": payee_ph, "memos": []})
            large_coins += 1
            ...
            if xck_spam_amount - dust_value - i > 0:
                payees.append({"amount": uint64(xck_spam_amount - dust_value - i), "puzzlehash": payee_ph, "memos": []})
            else:
                payees.append({"amount": uint64(dust_value), "puzzlehash": payee_ph, "memos": []})
